...
<form action="<?php echo $this->getUrl('page','save',['id'=>$page->id],true) ?>" method="POST">
	<table align="center" border="1" cellspacing="4">
		...
		<tr>
				...
				<td><input type="radio" name="page[status]" value="1" <?php if($page->getStatus($page->status)==1){echo "checked";} ?>>Active
				<input type="radio" name="page[status]" value="2" <?php if($page->getStatus($page->status)==2){echo "checked";} ?>>DeActive</td>
				</tr>
