...
class Controller_Category_Media extends Controller_Core_Action{
	...
	public function saveAction()
	{
		...
		if($request->isPost()){
			try {
				...
				$this->redirect('category_media','grid',['id' => $category_id],true);
	...
	{
		$this->setTitle('Category Media Edit');
		try{
			$request = $this->getRequest();
			...
			if(array_key_exists('media',$rows) && array_key_exists('small',$rows['media']))
			{
				...
				$this->redirect('category_media','grid',['id' => $categoryId],true);
