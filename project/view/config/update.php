...
<form action="<?php echo $this->getUrl('save','config',['id'=>$config->id],true) ?>" method="POST">
	<table align="center">
		...
		<tr>
				...
				<td><input type="radio" name="config[status]" value="1" <?php echo ($config->getStatus($config->status)=='Active')? 'checked' : ''; ?>>Active</td>
				<td><input type="radio" name="config[status]" value="2" <?php echo ($config->getStatus($config->status)=='Inactive')? 'checked' : ''; ?>>Inactive</td>
				</tr>
